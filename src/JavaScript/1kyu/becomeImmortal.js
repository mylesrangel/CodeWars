...
function elderAge(m,n,l,t) {
  ...
  let eldarAge = 0;
  for(let i = 0; i<m; i++){
    firstRowValue = firstRowValue + i;
  }
  console.log(firstRowValue);
  //multiply by n -1
  eldarAge = firstRowValue * (n);
  console.log(eldarAge);
  if(l > 0){
    //subtract ((m*n)-n) from the sum
    eldarAge = eldarAge - (m*n-n);
    console.log(eldarAge)
  }
  ///Figure out what t does and do that as well 
  if(t < eldarAge){
      eldarAge = eldarAge - t;
  }
  console.log(eldarAge);
...
console.log(elderAge(25,31,0,100007)) //1925
console.log(9300 - 1925);
